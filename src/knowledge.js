...
3. Interruption - Behavioral Health Mobile App (React Native, TypeScript, SQLite). A production mobile app implementing the JITAI (Just-In-Time Adaptive Intervention) framework from behavioral health research, with dynamic risk scoring, haptic feedback, and real-time state management across iOS and Android.
4. Retail Logistics Data Quality Analysis (Python, Pandas, scikit-learn, Power BI). Quantified the financial impact of poor data quality across 10,000+ logistics records using ML anomaly detection, identifying a $2.3M annual cost impact, and delivered data-governance recommendations from root-cause analysis.
...
6. FitFindr - Multi-Tool AI Agent for Secondhand Fashion (Python, Groq, LLaMA-3.3-70b, Gradio). A multi-tool agent that finds secondhand pieces and styles them, built on a planning loop with one real decision point (not a fixed pipeline). Three tools — listing search with weighted keyword scoring, outfit suggestion against the user's wardrobe, and a shareable caption generator — each handle their own failure modes to guarantee a useful response. Well-documented with state management, error handling, and an architecture diagram. Has a live demo.
...
Baruch College, City University of New York — pursuing a Bachelor of Business Administration in Computer Information Systems (CIS), Data Analytics concentration, expected May 2028. Also active in CodePath's AI201 program (RAG pipelines, multi-tool agents, safety guardrails) and Break Through Tech's Cornell Machine Learning Foundations program (the full ML lifecycle, supervised learning, deep learning, NLP, RAG, and AI agents).`,
  ...
  FACTS: [
    ...
    { k: "fitfindr fit findr fashion agent multi-tool secondhand outfit styling groq llama gradio", a: "**FitFindr** is a multi-tool AI agent for finding and styling secondhand fashion — built on a planning loop with one real decision point (not a fixed pipeline). It uses three tools (listing search, outfit suggestion, shareable caption) on Groq's LLaMA-3.3-70b with a Gradio UI, each tool absorbing its own failure modes. Has a [live demo](https://huggingface.co/spaces/adouu/fitfindr)." },
    ...
    { k: "project projects portfolio built work showcase", a: "Mamadou's flagship projects are two production RAG systems — **VibeFinder** and **The Unofficial Guide** — plus **Interruption** (a React Native behavioral-health app), a **$2.3M data-quality analysis**, a C++ **Hospital Management System**, and **FitFindr** (a multi-tool AI fashion agent). More on [GitHub](https://github.com/Mamadouba2004?tab=repositories)." },
  ],
